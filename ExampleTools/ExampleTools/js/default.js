	...
	app.onactivated = function (args) {
		if (args.detail.kind === activation.ActivationKind.launch) {
			if (args.detail.previousExecutionState !== activation.ApplicationExecutionState.terminated) {
				...
				// roaming data
				$doc.on('click', '#5', function () {
				    WinjsTools.roaming('test', 'Testing' + Math.random());
					$('#5-extra').text(WinjsTools.roaming('test')); 
				});

				$('#5-extra').text(WinjsTools.roaming('test'));

			    // purchase
                // (fails because isn't in the store)
				$doc.on('click', '#6', function () {
				    WinjsTools.purchase({
				        id: 'myPurchase',
				        debugMode: true
				    });
